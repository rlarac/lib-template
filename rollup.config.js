...
function genRollupObj(format, isGlobal = false) {
  function isESM() {
  ...
  return {
      ...
      plugins: [
        ...
        ts({
          ...
          tsconfigOverride: {
            ...
            include: ['src'],
            exclude: ['**/node_modules', '**/tests', '**/dist', 'playground'],
          },
        }),
        postcss({
          plugins: [
          ...
          extract:
            isESM() && resolve(packageFolder, 'dist/' + name + '.min.css'),
          inject: false,
        }),
        ...plugins,
      ],
    ...
    output: {
      ...
      name,
      entryFileNames:
        name + '.' + format + (isGlobal ? '.browser' : '') + '.bundle.js',
      extend: true,
      exports: 'auto',
    },
  };
